# 科研项目文档初始化 Agent Skill

[English](README.md)

这是一个跨 agent 使用的科研项目初始化 skill，用于生成证据边界清晰、实验可复现、写入范围安全，并带有主动项目访谈流程的文档体系。

可移植核心是 `SKILL.md`、`scripts/` 和 `assets/`。`agents/openai.yaml` 仅提供可选的 OpenAI/Codex UI 元数据，其他 agent 可以安全忽略。
